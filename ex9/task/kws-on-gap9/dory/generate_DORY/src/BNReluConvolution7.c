...
static void load_weights_async(Layer tile, Layer body, Layer layer, TileIndex index) {
  dma_transfer_async((DmaTransferConf) {
    .ext = dory_get_tile_3d(layer.addr.weights,
                            ...
                            body.weights.output_channel, 3*3, body.weights.input_channel,
                            3*3, 1,
                            0, 0, 0,
                            0, 0, 0,
                            8),
    ...
    .length_1d_copy = (int) tile.weights.output_channel * 8 * 3*3 / 8,
    .hwc_to_chw = 0,
    .stride_2d = 9,
    .stride_1d = 1,
    .dir = 1
  ...
  dma_transfer_1d_async((DmaTransferConf) {
    .ext = layer.addr.scale + 256*index.output_channel,
    .loc = tile.addr.scale,
  ...
  dma_transfer_1d_async((DmaTransferConf) {
    .ext = layer.addr.bias + 256*index.output_channel,
    .loc = tile.addr.bias,
    .length_1d_copy = tile.weights.output_channel * 4,
    .dir = 1
  });
...
void __attribute__ ((noinline)) BNReluConvolution7(void *args) {
  //////////////////////////////////////////////////////////////////////////
  ...
  const Layer layer = {
    .addr = {
      ...
      .scale = layer_args->L2_weights + 576,
      .bias = layer_args->L2_weights + 832,
  ...
  DoubleBuffer db_input = {
    .addrs = { l1_buffer + 0, l1_buffer + 0 + 6720 },
    .index = 0
  };
  DoubleBuffer db_output = {
    .addrs = { l1_buffer + 6728, l1_buffer + 6728 + 6400 },
    .index = 0
  };
  DoubleBuffer db_weights = {
    .addrs = { l1_buffer + 13136, l1_buffer + 13136 + 1088 },
    .index = 0
  };
  ...
  void * im2col = l1_buffer + 14248;
  void * pwt_buffer = im2col + 552;
  ...
  for(int iter=0; iter < total_tiles; iter++) {
    Address addr = {
      ...
      .scale = double_buffer_get_addr(db_weights) + 576,
      .bias = double_buffer_get_addr(db_weights) + 576 + 256,
